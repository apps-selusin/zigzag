...
//
// Table class for t04_depo
//
class ct04_depo extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 't04_depo';
		$this->TableName = 't04_depo';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t04_depo`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('t04_depo', 't04_depo', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// nama
		$this->nama = new cField('t04_depo', 't04_depo', 'x_nama', 'nama', '`nama`', '`nama`', 200, -1, FALSE, '`nama`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nama->Sortable = TRUE; // Allow sort
		$this->fields['nama'] = &$this->nama;

		// alamat
		$this->alamat = new cField('t04_depo', 't04_depo', 'x_alamat', 'alamat', '`alamat`', '`alamat`', 200, -1, FALSE, '`alamat`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->alamat->Sortable = TRUE; // Allow sort
		$this->fields['alamat'] = &$this->alamat;

		// kota
		$this->kota = new cField('t04_depo', 't04_depo', 'x_kota', 'kota', '`kota`', '`kota`', 200, -1, FALSE, '`kota`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kota->Sortable = TRUE; // Allow sort
		$this->fields['kota'] = &$this->kota;

		// propinsi
		$this->propinsi = new cField('t04_depo', 't04_depo', 'x_propinsi', 'propinsi', '`propinsi`', '`propinsi`', 200, -1, FALSE, '`propinsi`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->propinsi->Sortable = TRUE; // Allow sort
		$this->fields['propinsi'] = &$this->propinsi;

		// no_telp
		$this->no_telp = new cField('t04_depo', 't04_depo', 'x_no_telp', 'no_telp', '`no_telp`', '`no_telp`', 200, -1, FALSE, '`no_telp`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->no_telp->Sortable = TRUE; // Allow sort
		$this->fields['no_telp'] = &$this->no_telp;

		// no_fax
		$this->no_fax = new cField('t04_depo', 't04_depo', 'x_no_fax', 'no_fax', '`no_fax`', '`no_fax`', 200, -1, FALSE, '`no_fax`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "t03_lift") {
			$sDetailUrl = $GLOBALS["t03_lift"]->GetListUrl() . "?" . EW_TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$sDetailUrl .= "&fk_id=" . urlencode($this->id->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "t04_depolist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`t04_depo`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t04_depoview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t04_depoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "t04_depoadd.php?" . $this->UrlParm($parm);
		else
			$url = "t04_depoadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t04_depoedit.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t04_depoedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("t04_depoadd.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("t04_depoadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("t04_depodelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->no_fax->EditValue = $this->no_fax->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->no_fax->Exportable) $Doc->ExportCaption($this->no_fax);
				} else {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->no_fax->Exportable) $Doc->ExportField($this->no_fax);
					} else {
